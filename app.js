...
(function () {
  ...
  let navLinks = document.querySelector(".navbar__menus");
  let sideNavLinks = document.querySelector(".side__navbar--menus");
  let sideNav = document.getElementById("sidenav");
  let hamburgerBtn = document.querySelector(".hamburger");
  let closeBtn = document.querySelector(".closebtn");
  let sideNavSearch = document.querySelector(".side__navbar--search");

  const restaurantCard = (data) => {
    return `
    <div class="card__img--container">
      <img class="card__img" src="${data.img}" alt="${data.description}"/>
    </div>
    <div class="card__content">
      <h3 class="card__title">${data.title}</h3>
      <p class="card__price">${data.price}</p>
      <p class="card__description">${data.description}</p>
      <p class="card__category">Category: ${data.category}</p>
    </div>
    `;
  };
  ...
  const filterCards = (val) => {
    return menu.filter((menu) =>
      menu.title.toLocaleLowerCase().includes(val.toLocaleLowerCase())
    );
  };

  const filterCardsByCategory = (val) => {
    return menu.filter((menu) =>
      menu.category.toLocaleLowerCase().includes(val.toLocaleLowerCase())
    );
  };
  ...
  const renderCards = (cards) => {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      card.innerHTML = restaurantCard(cards[i]);
  ...
  searchInput.addEventListener(
    "input",
    debounce((e) => {
      renderCards(filterCards(e.target.value));
    }, 1000)
  );

  sideNavSearch.addEventListener(
    "input",
    debounce((e) => {
      renderCards(filterCards(e.target.value));
    }, 1000)
  ...
  navLinks.addEventListener("click", (e) => {
    ...
    renderCards(filterCardsByCategory(e.target.dataset.menu));
  });

  sideNavLinks.addEventListener("click", (e) => {
    if (e.target.dataset.menu === "all") {
      renderCards(menu);
      return;
    }
